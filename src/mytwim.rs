...
use hal::target::{twim0, P0};
use hal::target::TWIM1;
use nrf52840_hal::{
...
where
    T: Instance,
{
    pub fn new(twim: T, pins: i2cPins, frequency: Frequency) -> Self {
    ...
    pub fn write(&mut self, address: u8, buffer: &[u8]) -> Result<(), Error> {
        // hprintln!("my write method").unwrap();
        slice_in_ram_or(buffer, Error::DMABufferNotInDataMemory)?;

        if buffer.len() > target_constants::EASY_DMA_SIZE {
    ...
    pub fn write_then_read(
        ...
        if wr_buffer.len() > target_constants::EASY_DMA_SIZE {
            return Err(Error::TxBufferTooLong);
        }

        if rd_buffer.len() > target_constants::EASY_DMA_SIZE {
            return Err(Error::RxBufferTooLong);
        }
